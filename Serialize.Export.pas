{ =============================================================================
  Serialize.Export - TDataSetSerialize (DataSet -> JSONObject/JSONArray)
  ...
  Absorvido de FontesReferencias/dataset-serialize/src/DataSet.Serialize.Export.pas
  (namespace DataSet.Serialize.Export -> Serialize.Export; uses internos
  DataSet.Serialize.Utils/.Consts/.UpdatedStatus/.Config -> Serialize.Utils/
  .Consts/.UpdatedStatus/.Config). Conteudo logico verbatim; UNICA mudanca de
  placement: EDataSetSerializeException (Exception cru) substituida por
  ESerializeException (Exceptions.Serialize, herda EExceptionBase).

  Changelog (file):
  - 1.0.0 (13/07/2026): versao inicial (FASE 5 Onda 9) - absorcao TOTAL do
    dataset-serialize, header v3; EDataSetSerializeException.CreateFmt ->
    ESerializeException.Create(Format(...), ERR_SERIALIZE_FIELD_TYPE_NOT_FOUND)
    nos 2 pontos de "tipo de campo sem mapeamento JSON".
  ============================================================================= }
unit Serialize.Export;
...
{$ENDIF}
  Serialize.Utils, Serialize.Consts, Serialize.UpdatedStatus, Serialize.Config, Exceptions.Serialize;
    ...
    begin
      ...
      begin
        case ADataSet.Fields[0].DataType of
          ...
          raise ESerializeException.Create(Format(FIELD_TYPE_NOT_FOUND, [ADataSet.Fields[0].FieldName]), ERR_SERIALIZE_FIELD_TYPE_NOT_FOUND);
        end;
      end
      else
...
begin
  ...
  begin
    ...
    case LField.DataType of
      ...
      { bug-977 - ftShortint/ftLongWord/ftWord/ftByte SAO validos em ambos os
        compiladores (TFieldType, DB/Data.DB) - a exclusao FPC aqui era um
        pressuposto incorreto de que estes subtipos "nao ocorrem" sob FPC.
        Zeos (FPC) x MariaDB devolve uma coluna BOOLEAN/TINYINT(1) como
        ftShortint (nao ftBoolean) na copia desconectada (CreateDisconnectedCopy,
        Connections.pas, nao remapeia estes subtipos sob FPC) - sem este ramo
        cai no 'else' e lanca ESerializeException "Cannot find type for field"
        (test_serialize.dpr, achado real, nao mascarado). Unificado
        cross-compiler (idem Serialize.Import.pas). }
      TFieldType.ftInteger, TFieldType.ftSmallint, TFieldType.ftAutoInc,
      ...
      TFieldType.ftVarBytes, TFieldType.ftBytes:
        ...
        raise ESerializeException.Create(Format(FIELD_TYPE_NOT_FOUND, [LKey]), ERR_SERIALIZE_FIELD_TYPE_NOT_FOUND);
    end;
  end;
