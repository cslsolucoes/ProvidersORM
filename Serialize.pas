{ =============================================================================
  Serialize - TDataSetSerializeHelper (class helper for TDataSet)
  ...
  Absorvido de FontesReferencias/dataset-serialize/src/DataSet.Serialize.pas
  (namespace DataSet.Serialize -> Serialize; uses internos DataSet.Serialize.
  Language/.Config/.Export/.Import -> Serialize.Language/.Config/.Export/
  .Import). Conteudo logico verbatim - API publica do modulo (`uses Serialize;`):
  TDataSetSerializeHelper = class Helper for TDataSet expoe ToJSONObject/
  ToJSONArray/ToJSONObjectString/ToJSONArrayString/SaveStructure/LoadStructure/
  LoadFromJSON/MergeFromJSONObject/ValidateJSON directamente em qualquer TDataSet
  (Zeos/FireDAC/BufDataset/ClientDataSet/...).

  Changelog (file):
  - 1.0.0 (13/07/2026): versao inicial (FASE 5 Onda 9) - absorcao TOTAL do
    dataset-serialize, header v3. Sem gate USE_* proprio (modulo sempre presente,
    como Commons/Exceptions/Connections); so o adapter opcional
...
{$ELSE}
  System.JSON, Data.DB,
{$ENDIF}
  Serialize.Language, Serialize.Config;

type
  TLanguageType = Serialize.Language.TLanguageType;
  TDataSetSerializeConfig = Serialize.Config.TDataSetSerializeConfig;
  TCaseNameDefinition = Serialize.Config.TCaseNameDefinition;
...
{$ELSE}
  System.SysUtils,
{$ENDIF}
  Serialize.Export, Serialize.Import;
