{ =============================================================================
  Serialize.Utils - Helpers de nomenclatura/estrutura de campo + hack FPC
  ...
  Absorvido de FontesReferencias/dataset-serialize/src/DataSet.Serialize.Utils.pas
  (namespace DataSet.Serialize.Utils -> Serialize.Utils; uses internos
  DataSet.Serialize.Consts/.Config -> Serialize.Consts/.Config). Conteudo
  logico verbatim; UNICA mudanca de placement: EDataSetSerializeException
  (Exception cru) removida daqui e substituida por ESerializeException
  (Exceptions.Serialize, herda EExceptionBase - regra v3 "nunca lancar
  Exception cru"; ver Exceptions.Serialize para a faixa 44 e o mapeamento
  de codigos).

  Changelog (file):
  - 1.0.0 (13/07/2026): versao inicial (FASE 5 Onda 9) - absorcao TOTAL do
    dataset-serialize, header v3; EDataSetSerializeException -> ESerializeException
    (Exceptions.Serialize) + raise com ERR_SERIALIZE_SIZE_NOT_DEFINED.
  ============================================================================= }
unit Serialize.Utils;
...
type
  {$IF DEFINED(FPC)}
  // hack para conhecer o DetailDataset. delphi tem este metodo mas FreePascal nao.
...
uses
  Serialize.Consts, Serialize.Config, Exceptions.Serialize;
...
begin
  ...
  if (AFieldStructure.FieldType in [ftString, ftWideString]) and (AFieldStructure.Size <= 0) then
    raise ESerializeException.Create(Format(SIZE_NOT_DEFINED_FOR_FIELD, [AFieldStructure.FieldName]), ERR_SERIALIZE_SIZE_NOT_DEFINED);
end;
