{ =============================================================================
  Serialize.Import - TJSONSerialize (JSONObject/JSONArray -> DataSet)
  ...
  Absorvido de FontesReferencias/dataset-serialize/src/DataSet.Serialize.Import.pas
  (namespace DataSet.Serialize.Import -> Serialize.Import; uses internos
  DataSet.Serialize.Language/.Utils/.Consts/.Config/.UpdatedStatus -> Serialize.
  Language/.Utils/.Consts/.Config/.UpdatedStatus). Conteudo logico verbatim -
  merge por object_state ('MODIFIED'/'INSERTED'/'DELETED'/'UNMODIFIED',
  Serialize.UpdatedStatus), Locate por pfInKey, LoadStructure via
  TFieldStructure. UNICA mudanca de placement: EDataSetSerializeException
  (Exception cru) substituida por ESerializeException (Exceptions.Serialize,
  herda EExceptionBase).

  Changelog (file):
  - 1.0.0 (13/07/2026): versao inicial (FASE 5 Onda 9) - absorcao TOTAL do
    dataset-serialize, header v3; todos os pontos EDataSetSerializeException.
    Create/.CreateFmt -> ESerializeException.Create(Format(...), ERR_SERIALIZE_*)
    com o codigo correspondente por constante de mensagem (ver Exceptions.Serialize).
  ============================================================================= }
unit Serialize.Import;
...
{$ELSE}
  ...
  {$ENDIF}
{$ENDIF}
  Serialize.Language, Serialize.Utils;
...
{$ENDIF}
  Serialize.Consts, Serialize.Config, Serialize.UpdatedStatus, Exceptions.Serialize;
    ...
    begin
      ...
      {$ENDIF}
        ...
        case LField.DataType of
          ...
          { bug-977 - ver comentario completo em Serialize.Export.pas
            (DataSetToJSONObject): ftShortint/ftLongWord/ftWord/ftByte sao
            validos sob FPC (DB.TFieldType) - unificado cross-compiler para
            simetria com o eixo Export (mesma classe de campo, mesmo tipo de
            origem Zeos/MariaDB). }
          TFieldType.ftInteger, TFieldType.ftSmallint,
          TFieldType.ftWord{$IFDEF FPC}{$IF FPC_FULLVERSION >= 30300}, TFieldType.ftShortint, TFieldType.ftLongWord, TFieldType.ftByte{$ENDIF}{$ELSE}, TFieldType.ftShortint, TFieldType.ftLongWord, TFieldType.ftByte{$ENDIF}:
          ...
          TFieldType.ftVarBytes, TFieldType.ftBytes:
            ...
            raise ESerializeException.Create(Format(FIELD_TYPE_NOT_FOUND, [LField.FieldName]), ERR_SERIALIZE_FIELD_TYPE_NOT_FOUND);
        end;
      end;
      ADataSet.Post;
    end;
...
begin
  if ADataSet.Fields.Count <> 1 then
    raise ESerializeException.Create(Format(INVALID_FIELD_COUNT, [ADataSet.Name]), ERR_SERIALIZE_INVALID_FIELD_COUNT);
  if not ADataSet.Active then
    ADataSet.Open;
  ADataSet.Append;
...
begin
  if Assigned(FJSONObject) then
    ...
    raise ESerializeException.Create(JSON_NOT_DIFINED, ERR_SERIALIZE_JSON_NOT_DEFINED);
end;
...
begin
  ...
  if ADataSet.Fields.Count = 0 then
    raise ESerializeException.Create(DATASET_HAS_NO_DEFINED_FIELDS, ERR_SERIALIZE_NO_DEFINED_FIELDS);
...
{$IF NOT DEFINED(FPC)}
  if AJSONValue.TryGetValue<string>(FIELD_PROPERTY_DATA_TYPE, LStrTemp) then
    ...
    raise ESerializeException.Create(Format('Attribute %s not found in json!', [FIELD_PROPERTY_DATA_TYPE]), ERR_SERIALIZE_GENERIC);
  ...
  if AJSONValue.TryGetValue<string>(FIELD_PROPERTY_FIELD_NAME, LStrTemp) then
    ...
    raise ESerializeException.Create(Format('Attribute %s not found in json!', [FIELD_PROPERTY_FIELD_NAME]), ERR_SERIALIZE_GENERIC);
...
begin
  if Assigned(FJSONObject) then
    raise ESerializeException.Create(TO_CONVERT_STRUCTURE_ONLY_JSON_ARRAY_ALLOWED, ERR_SERIALIZE_STRUCTURE_JSON_ARRAY_ONLY)
  else if Assigned(FJSONArray) then
...
begin
  ...
  case ALang of
    ptBR:
      begin
        LError := ADisplayLabel + ' nao foi informado(a)';
        {$IF DEFINED(FPC)}
...
begin
  if ADataSet.Active then
    raise ESerializeException.Create(DATASET_ACTIVATED, ERR_SERIALIZE_DATASET_ACTIVATED);
  if ADataSet.FieldCount > 0 then
    raise ESerializeException.Create(PREDEFINED_FIELDS, ERR_SERIALIZE_PREDEFINED_FIELDS);
  for I := 0 to Pred(AJSONArray.Count) do
